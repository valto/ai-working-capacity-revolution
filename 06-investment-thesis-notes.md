...
| Author's own published cooperative-cost figure | ~€42/member/month (50 members) | OBSERVED FACT that this figure appears in Valto Loikkanen's LinkedIn article "So, the majority agrees..." (published Aug 5 2026, verified verbatim) — but NOT independently verified as the correct market cost itself |
| Later recomputation using DGX Spark figures above | ~€19.50–23/member/month (50 members, 10 units) | DERIVED CALCULATION, same background model |

**Unreconciled discrepancy (shown, not hidden):** the published €42/member/month figure and the later €19.50–23/member/month recomputation differ by roughly 2x. Plausible drivers, none confirmed: different hardware assumption (the published figure may assume a costlier or more capable device than a DGX Spark), a lower assumed utilization rate, different financing terms/tenor, a higher assumed electricity price, or — most likely — the published figure includes admin/coordination/networking overhead that the later recomputation excludes ("before admin/networking" is stated explicitly in the source model). This is flagged as an open reconciliation item for the workbook, not resolved here.

**Demand-side factors that would justify it:** enough members with genuinely complementary usage schedules (peak-shaving each other) to keep aggregate utilization high; a shared trust/governance structure that can fairly allocate scarce capacity at peak; a cooperative's usage profile skewing toward the copilot/delegated bands rather than occasional chat (per-capita chat-only usage does not amortize shared hardware well).

**Key risks:** free-rider and allocation-fairness problems (heavy users vs. light payers); coordination/admin overhead that is real but excluded from the headline per-token number above; membership churn destabilizing the utilization assumption the whole model depends on; the ~2x unresolved discrepancy above signals the whole cooperative-cost claim needs re-verification before being used as a marketing or policy number.

**What would have to be true for this tier to make sense:** the €19.50–42/member/month range (whichever is eventually confirmed) must beat the member's realistic alternative — either individual home ownership (Tier 1) or metered API/cloud access — at that member's actual usage level, not at an assumed average.

*Non-advice boundary: the cooperative figures above are competing scenario computations from the same unresolved model, not a validated cooperative-formation business case.*
...
| Cost per M tokens (GB300-class, interactivity-dependent) | ~$0.065–$3.3/M tokens across the interactivity curve (low-latency to high-throughput settings) | OBSERVED FACT-adjacent — interpolated from SemiAnalysis InferenceX's live GB200-vs-GB300 DeepSeek-R1 comparison page, checked 2026-08-12; the specific oft-cited "$0.123/M tokens" point figure is itself UNVERIFIABLE as an exact published number |
