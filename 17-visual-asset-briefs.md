...
**Status:** Diagrams 1–10 have been built as finished PNG images (see `assets/diagrams/`); the briefs below remain the specification each was built from, kept for anyone editing or re-deriving them. **Diagram 11** ("Creating Growth Based on New Value," added 2026-08-14, alongside the new "From AI working capacity to new value" conceptual bridge section) is the author's own pre-existing diagram, supplied directly and used as-is — see `assets/diagrams/diagram-11-creating-growth-from-new-value.png` (provenance-framed for this package — evidence-class tag, attribution, source note, and standard footer added around the original image; the unmodified original supplied by the author is kept alongside it as `diagram-11-creating-growth-from-new-value.jpg`). Its brief below describes that image's actual structure rather than a designer specification to build from scratch, since the image already exists.
...
**Status:** Added 2026-08-14, alongside the new "From AI working capacity to new value" conceptual bridge section (placed between Parts IV and V of the whitepaper). Unlike Diagrams 1–10, this is the author's own **pre-existing diagram, supplied directly and used as-is** — see `assets/diagrams/diagram-11-creating-growth-from-new-value.png` (provenance-framed for this package — evidence-class tag, attribution, source note, and standard footer added around the original image; the unmodified original supplied by the author is kept alongside it as `diagram-11-creating-growth-from-new-value.jpg`). This entry describes that image's actual structure (for editors, alt-text, and consistency checking), not a from-scratch designer brief.

**Actual type:** A two-column, bottom-anchored conversion diagram — not a horizontal chain like Diagram 1. Two equal-width vertical columns ("Creating New Value" on the left, "Growth Based on New Value" on the right) each run three boxes, both columns converging into one shared horizontal band, "New Value," at the bottom. Grey downward arrows in the left column and grey upward arrows in the right column both point toward that shared band — visually stating that new value is created by descending the left column, and growth is created by ascending the right column, out of the same underlying value.

**What it shows:** The same core distinction as the whitepaper's "From AI working capacity to new value" section — that creating new value and creating growth from that value are two separate, sequential processes, neither of which is guaranteed by the other, and neither of which this paper's cost/capacity model (Parts I–IV) performs on its own.

**Exact content, box by box (top to bottom, left column then right):**
- **Idea / Invention** — "Insight into something new or targeting an existing solution to a new audience as a new offering."
- **Innovation** — "The process of development and implementation."
- **Verifying New Value** — "New value exists only when someone (typically a customer) is willing to pay for, use, or commit to it."
- **New Value** (shared bottom band, highlighted in blue, spanning both columns) — "What, for Whom, How, and how Verified."
- **Value Proposition & Communication** — "(Marketing = Scaled Sales) — making new value visible and communicating it attractively to the target group."
- **Revenue Model** — "How new value is exchanged for results/income."
- **Productivity (Output / Inputs)** — "How efficiently value is produced relative to the inputs used."

**How this maps onto the whitepaper's own chain:** the left column ("Creating New Value") corresponds to the whitepaper section's idea/invention → innovation → verified new value segment; the right column ("Growth Based on New Value") corresponds to its value proposition and communication → revenue model → productivity segment. The whitepaper's earlier steps — electricity → compute infrastructure → AI working capacity → useful work outputs → human and organizational integration — are upstream of this diagram entirely; this diagram begins at "idea/invention," i.e. at the point where a use for that capacity has already been identified. It should be captioned as such wherever it's placed, so a reader doesn't expect the AI-cost chain to appear inside this image.

**Boundary note for placement:** this diagram is the author's own general business/innovation framework, applicable well beyond AI, and predates this whitepaper. It is **[INTERPRETATION]** in the same sense as the rest of this section — a conceptual lens, not a numerical finding — and should carry that tag plus a brief note that it is a general framework applied here to the AI-working-capacity question, not derived from this paper's cost data.

**Evidence-class labeling:** The entire diagram is **[INTERPRETATION]** — the author's own general conceptual framework, applied to this paper's subject matter, not derived from any named executive's statement or from this paper's cost workbooks.
