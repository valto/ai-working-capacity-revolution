...
### Fixed
- Fixed an arithmetic error in the delegated-agent band's Hyperscale $/AI-working-hour figure: whitepaper §25 and `13-slide-deck-outline.md` (and the deck built from it, before this fix) cited a stale $0.0252/hr figure and a "~150x" spread, which contradicted Section 17's own matrix, the canonical CSV, and the correct arithmetic (400,000 tokens/hr × $0.133/M = $0.0532/hr). Corrected to the right $0.0532/hr and "~68x" figure in both files; `19-slide-deck.pptx` already had the correct figure on slide 13 and did not need changing. Caught during the Finnish translation's cross-check against the canonical model — fixed in both languages simultaneously.
