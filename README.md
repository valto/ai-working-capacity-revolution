...
| `index.html`, `*.html` | The GitHub Pages site — every markdown document above has a matching native HTML page (built from it, not a duplicate) so the whole package can be read inside the site without jumping to GitHub's markdown viewer | Anyone reading the live site at [valto.github.io/ai-working-capacity-revolution](https://valto.github.io/ai-working-capacity-revolution/) |
| `diagrams.html` | A gallery page for all 11 diagrams, with captions and alt text, linking to `17-visual-asset-briefs.html` for full specs | Readers browsing the visual set without leaving the site |
| `perspectives.html`, `perspectives/*.yaml` | The Perspectives/Writeups index — original, independent, and critical publications derived from or challenging this research, filterable by type/topic. See `perspectives/README.md` for the entry schema | Anyone tracking the broader discussion around this work, or submitting their own writeup |
| `docs/backlink-discovery.md` | Design (not yet implemented) for an automated backlink-discovery → classification → Perspectives pipeline | Maintainers considering Ahrefs/Semrush API integration |
| `tools/build-pages.py`, `tools/page-template.html` | Generates every markdown-sourced `*.html` page from its `.md` source, with cross-references rewritten to stay inside the site | Maintainers, after any markdown content change — see below |
| `tools/build-perspectives.py`, `tools/perspectives-template.html` | Generates `perspectives.html` from every `perspectives/*.yaml` entry | Maintainers, after adding/editing a perspective |

## Rebuilding the Pages site after a content change

Every `*.html` file at the repo root except `index.html` and `diagrams.html` (hand-authored) and `perspectives.html` (generated by a separate script, below) is generated from its matching `.md` file — **do not hand-edit a generated `.html` file**; edit the markdown source and rebuild:

```bash
python3 tools/build-pages.py
```

This regenerates all pages and rewrites cross-references between them (a link to another tracked `.md` file becomes a link to that file's `.html` page) so reading stays inside the Pages site rather than jumping to GitHub's markdown viewer. Requires `pandoc` on `PATH`.

After adding or editing a `perspectives/*.yaml` entry, rebuild the Perspectives index:
...
- **Resolved (2026-08-14):** this package is now deployed as a GitHub Pages site at [valto.github.io/ai-working-capacity-revolution](https://valto.github.io/ai-working-capacity-revolution/), tracking `main` — see the Status line above for how the live site relates to the last tagged release.
