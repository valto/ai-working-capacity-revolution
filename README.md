...
| `05-workbook-token-factory-scenarios.md` | Release Asset #10 — token/AI-factory scenarios, the €42-vs-€19.50–25/member/month cooperative-cost reconciliation, and the Bitcoin-mining energy-monetization comparison | Cooperative/community infrastructure planners |
...
Known items flagged transparently within the package itself (not resolved, by design — see the relevant sections for detail):

- The cooperative-cost discrepancy between the author's published ~€42/member/month figure and this package's ~€19.50–25/member/month recomputation is investigated but **not resolved** — see `05-workbook-token-factory-scenarios.md` §1 and `11-appendix-assumption-register.md` row 19.
- Several hardware/price inputs (Professional-tier HGX B300 node pricing/power, hyperscale rack capex, the illustrative €25,000 humanoid robot price) have no public source and are flagged ⚠ throughout as placeholders, not verified figures.
- One commonly-cited figure ("$0.123/M tokens" for GB300-class inference) could not be located verbatim on its purported source (SemiAnalysis InferenceX) during this verification pass — see the source register for the nearest independently-checkable range.
- This package has **not** been deployed to a public site (no GitHub Pages, no PDF export yet) — it exists as a reviewable Markdown package in this repository pending author review.
